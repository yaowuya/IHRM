  ...
  <div class="app-container">
    <el-form :inline="true" :model="queryForm" class="pl-5">
      <el-form-item label="角色名">
        <el-input v-model="queryForm.name" placeholder="请输入角色名" clearable />
      </el-form-item>
      <el-form-item>
    ...
    >
      <el-form ref="ruleForm" :model="ruleForm" status-icon :rules="rules" label-width="100px">
        ...
        <el-form-item label="角色说明" prop="desc">
          <el-input v-model="ruleForm.desc" type="textarea" placeholder="请输入角色说明" />
        </el-form-item>
      </el-form>
  ...
  methods: {
    ...
    remove(row) {
      this.$confirm('此操作将永久删除该记录, 是否继续?', '提示', {
        ...
        this.loading = true
        await this.$api.common.delete('roles', { _id: row._id })
        this.loading = false
    ...
    async edit() {
      ...
      try {
        await this.$api.common.edit('roles', this.ruleForm)
        this.centerDialogVisible = false
    ...
    async add() {
      const loading = this.localLoading()
      try {
        await this.$api.common.add('roles', this.ruleForm)
        this.centerDialogVisible = false
